...
Codex reads `AGENTS.md` from repo root down to the working directory and discovers repo-local skills from `.agents/skills/`.

- `$ios-conventions` - repository-wide Swift, SwiftUI, MVVM, DI, navigation, and doc-sync guardrails
- `$ios-build-verify` - choose the smallest sufficient `xcodebuild` or Taskfile validation
- `$ios-feature-delivery` - coordinate multi-layer feature work and pull in companion skills
- `$ios-contract-alignment` - own backend-facing DTO, endpoint, API client, and auth-boundary decisions
- `$ios-testing` - choose between unit and UI coverage, XCTest baselines, and Swift Testing adoption
- `$swiftui-design-system` - own shared SwiftUI design language, components, and screen-pattern reuse

Keep the Claude project skills in `.claude/skills/` semantically aligned with these Codex skills where topics overlap.
