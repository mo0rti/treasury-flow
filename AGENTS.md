...
Codex reads `AGENTS.md` from repo root down to the working directory, so defer to the
platform-level file when working inside a specific platform folder.

Generated-workspace assumption:

- Open the **generated repository root** as the workspace when using Codex.
- The generated repo keeps shared Codex skills at `.agents/skills/` in the repo root.
...
Each platform has its own `AGENTS.md` with detailed patterns:
- `backend/AGENTS.md` - Spring Boot 4, security, JPA, Flyway
- `web-user-app/AGENTS.md` - Next.js, next-intl, marketing + dashboard flows
- `web-admin-portal/AGENTS.md` - Admin web portal, credentials auth, management tables
- `mobile-android/AGENTS.md` - Compose, Hilt, Retrofit 3, Room
- `mobile-ios/AGENTS.md` - SwiftUI, async/await, Keychain, SwiftData, local iOS skills, mobile-ios/docs/ references
